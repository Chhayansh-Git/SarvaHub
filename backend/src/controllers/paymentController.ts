...
export async function handleStripeWebhook(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (!stripe) {
            // Dev-mode only: process without signature verification
            if (config.nodeEnv !== 'development') {
                throw Errors.internal('Stripe is not configured. Cannot process webhooks in non-development mode.');
            }
            console.warn('[StripeWebhook] ⚠️  Stripe not configured — processing in DEV mode without signature verification.');
        ...
        try {
            ...
            throw Errors.badRequest('Webhook signature verification failed.');
        }
...
export async function handleRevenueCatWebhook(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (!expectedSecret) {
            ...
            if (config.nodeEnv !== 'development') {
                throw Errors.internal('RevenueCat webhook secret is not configured.');
            }
            console.warn('[RevenueCat] ⚠️  No webhook secret configured — processing in DEV mode.');
        } else {
