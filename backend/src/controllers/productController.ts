...
export async function getProducts(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const { pipeline, page, limit } = await buildProductFilterPipeline(query);
...
export async function getProductFilters(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const { pipeline } = await buildProductFilterPipeline(query);
        const result = await Product.aggregate(pipeline);
...
function generateSimpleEmbedding(text: string): number[] {
    // Simple bag-of-characters hash embedding (placeholder)
